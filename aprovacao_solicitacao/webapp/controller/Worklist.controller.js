sap.ui.define([
    ...
    return BaseController.extend("vicente.desafio5p.aprovacaosolicitacao.controller.Worklist", {
        ...
        buscarPreco :  function (){
            ...
            if(aItems.length > 0){
                aItems.forEach(async function(aEntry,i,array){
                    ...
                    try {
                        model.setProperty(path + '/PrecoNovo','Aguarde...');
                        let request = await this.ajaxRequest(material.CodigoExterno);
                        let dados = request[0].d;
                        let preco = dados.UnitPrice;
